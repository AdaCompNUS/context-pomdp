...
class CrowdAgent:
    ...
    def get_bounding_box_corners(self):
        ...
        half_y_len = bbox.extent.y + 0.2 #+ 0.5
        half_x_len = bbox.extent.x + 0.2 #+ 0.5

        # if self.agent_tag == "Bicycle":
        #     half_y_len = bbox.extent.y * 1.5
...
def in_bounds(position):
    ...
    return map_x_min <= position.x <= map_x_max and map_y_min <= position.y <= map_y_max
...
if __name__ == '__main__':
    ...
    sidewalk = carla.Sidewalk(
        occupancy_map,
        carla.Vector2D(map_x_min, map_x_max), carla.Vector2D(map_y_min, map_y_max), #carla.Vector2D(-200, -200), carla.Vector2D(200, 200),
        3.0, 0.1,
        10.0)
    ...
    gen_areas = [[map_x_min, map_x_min+gen_area_size, map_y_min, map_y_max],
    [map_x_min, map_x_max, map_y_min, map_y_min+gen_area_size],
    [map_x_max-gen_area_size, map_x_max, map_y_min, map_y_max],
    ...
    while True:
        ...
        while len(crowd_agents) < NUM_AGENTS:
            ...
            while True:
                if initial_generation:
                    position = carla.Vector2D(random.uniform(map_x_min, map_x_max), random.uniform(map_y_min, map_y_max))
                else:
                    area_idx = (area_idx+1)%4
        ...
        for (i, crowd_agent) in enumerate(crowd_agents):
            ...
            if pref_vel:
                ...
                bbx_corners = crowd_agent.get_bounding_box_corners()
                draw_box(bbx_corners)
                gamma.set_agent_bounding_box_corners(i, bbx_corners) ### update agent bounding box corners

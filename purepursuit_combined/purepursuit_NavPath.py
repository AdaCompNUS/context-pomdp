...
class Pursuit(object):
    ...
    def __init__(self, reverse=False):
        ...
        self.tm2 = rospy.Timer(rospy.Duration(0.2), self.cb_pose_timer)
        ...
        self.publish_path()
        self.tm = rospy.Timer(rospy.Duration(0.05), self.cb_vel_timer)

        self.speed_limit = SPEED * 5

    def cb_vel_timer(self, event):
        self.pub_vel.publish(self.curr_vel)
        return

    def cb_speed_limit(self, msg):
        self.speed_limit = 1.0 #msg.data

    def cb_pose_timer(self, event):
        #print self.tf.frameExists(ROBOT_FRAME)
        #print self.tf.frameExists(MAP_FRAME)
        if self.tf.frameExists(ROBOT_FRAME): # and self.tf.frameExists(MAP_FRAME):
            t = self.tf.getLatestCommonTime(MAP_FRAME, ROBOT_FRAME)
            position, quaternion = self.tf.lookupTransform(MAP_FRAME, ROBOT_FRAME, t)
        ...
        self.curr_vel = vel
        self.publish_path()
        self.publish_pursuit_line(p, a, MAP_FRAME)

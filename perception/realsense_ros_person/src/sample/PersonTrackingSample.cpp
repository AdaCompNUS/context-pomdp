...
PersonTrackingSample::PersonTrackingSample() 
{
...
void PersonTrackingSample::InitMessaging(ros::NodeHandle& nodeHandle)
{
  ...
  mTrackingOutputPublisher = it.advertise("person_bbox",1);
...
//    ROS_INFO_STREAM("Received person tracking output message. Number of people: " << msg.frameData.numberOfUsers);
  ...
  mTrackingOutputPublisher.publish(cv_bridge::CvImage(header, "bgr8", colorImage).toImageMsg());
}

...
namespace RVO {
	...
	size_t RVOSimulator::addAgent(const Vector2 &position, float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed, float max_tracking_angle, Vector2 heading, const Vector2 &velocity, std::string tag)
	{
		Agent *agent = new Agent(this);

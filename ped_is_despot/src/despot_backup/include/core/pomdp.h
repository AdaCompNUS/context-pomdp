...
public:
	...
	/**
	 * Determistic simulative model for POMDP; This function is problem specified. 
	 * In this class, it directly calls the original Step() function in this class, 
	 * so that it can be treated as it does not use the importance sampling step when
	 * its child class does not implement this virtual function.
	 */
	 ...
	 * return features.
	 */
	virtual vector<double> Feature(const State& state) const;
	...
	//virtual double ImportanceWeight(State* particle, double total_weight, int particle_size);
	 virtual vector<double> ImportanceWeight(vector<State*> particles) const;


	 /**
	 * To print the particles, for testing purpose
	 */
	 virtual void PrintParticles(const vector<State*> particles,ostream& out = cout) const;
};

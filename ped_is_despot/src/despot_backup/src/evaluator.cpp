...
bool Evaluator::RunStep(int step, int round) {
	...
	*out_ << "-----------------------------------Round " << round
	...
	if (state_ != NULL) {
			vector <double> cur_step_feature;
			int save_step=40;
			if(step < save_step){
				cur_step_feature = model_->Feature(*state_);
				if(cur_step_feature.size() != 0) {
					round_features_.push_back(cur_step_feature);
					round_dis_rewards_.push_back(total_discounted_reward_ - Discount(step_) * reward);
					round_undis_rewards_.push_back(total_undiscounted_reward_ - reward);
				}
			}
			else if( (step >=89) && ( step < round_features_.size() + 89) ){
				round_dis_rewards_end_.push_back(total_discounted_reward_);
				round_undis_rewards_end_.push_back(total_undiscounted_reward_);
			}
	}
...
double POMDPEvaluator::EndRound() {
	...
	while(round_dis_rewards_end_.size()<round_features_.size()) {
		round_dis_rewards_end_.push_back(total_discounted_reward_);
		round_undis_rewards_end_.push_back(total_undiscounted_reward_);
	}
	for (int i = 0; i < round_features_.size(); ++i){
		*out_ <<"Feature and value : ";
		for (int j = 0; j < round_features_.at(i).size(); ++j){
			*out_<< round_features_.at(i).at(j)<<" ";
		}
		*out_<< round_dis_rewards_end_.at(i) - round_dis_rewards_.at(i) <<" "<< round_undis_rewards_end_.at(i) - round_undis_rewards_.at(i)<<" ";
		*out_<< total_discounted_reward_ - round_dis_rewards_.at(i) <<" "<< total_undiscounted_reward_ - round_undis_rewards_.at(i)<<" ";
		*out_<< total_discounted_reward_<<" "<< total_undiscounted_reward_<<" "<<i<<endl;
	}

	for (int i = 0; i < round_features_.size(); ++i){
		round_features_.at(i).clear();
	}
	round_features_.clear();
	round_dis_rewards_.clear();
	round_undis_rewards_.clear();
	round_dis_rewards_end_.clear();
	round_undis_rewards_end_.clear();

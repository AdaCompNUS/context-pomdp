...
State Pedestrian::step(State state, double rNum, int action, double& reward, uint64_t& obs) {
	...
	int add_vel=(4*Size*Size)*rob_Vel;
	int add_Y1=(4*Size)*robY;
	int add_Y2=4*pedY;
	int add_X=pedX;
	...
	//obs=rob_vel*500+robY*41+pedX*11+pedY;
...
double Pedestrian::fringeUpperBound(State s)  {
	NToState(s, dummyState);
	int d = (int)(fabs(Size - dummyState->RobPos.Y)) / 3;
...
{
	return 4*Size*Size*3*4+1;  //plus one dummy state	
}
int Pedestrian::StateToN(Pedestrian_State* teststate) const
{
	int add_goal=(4*Size*Size*3)*(teststate->Goal);
	int add_vel=(4*Size*Size)*teststate->Vel;
	int add_Y1=(4*Size)*teststate->RobPos.Y;
	int add_Y2=4*teststate->PedPos.Y;
...
void Pedestrian::NToState(int n,Pedestrian_State*teststate) const
{
	...
	int N=n;
	//Pedestrian_State*teststate=MemoryPool.Allocate();
	teststate->Goal=(N/(4*Size*Size*3));
	N=N%(4*Size*Size*3);
	teststate->Vel=N/(4*Size*Size);
	teststate->RobPos.X =1;
	N=N%(4*Size*Size);
	teststate->RobPos.Y=N/(4*Size);
	N=N%(4*Size);
	teststate->PedPos.Y=N/4;
	N=N%4;

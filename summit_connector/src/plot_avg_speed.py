...
def read_avg_speed(path):
    ...
    with open(path) as f:
        ...
        while line:
            ...
            global_total_speed += (s[10] + s[11] + s[12]) / (s[7] + s[8] + s[9])
            global_num += 1
            
            if int(t) >= last_time + INTERVAL - WINDOW and int(t) < last_time + INTERVAL + WINDOW:
                window_total_speed[0] += s[10] / s[7]
                window_total_speed[1] += s[11] / s[8]
                window_total_speed[2] += s[12] / s[9]
                window_num += 1
        ...
        print(path)
        print(global_total_speed / global_num)
        
    return data

data_gamma = read_avg_speed('{}_gamma.txt'.format(DATA))
data_simple = read_avg_speed('{}_simple.txt'.format(DATA))

...
class SummitDQL(Summit):
    ...
    def pixel(self, point):
        return (int((point.y - self.bounds_min.y) / self.resolution), int((self.bounds_max.x - point.x) / self.resolution))

    def create_frame(self):
        return np.zeros((int(2 * self.range / self.resolution), int(2 * self.range / self.resolution), 3), dtype = np.uint8)
    ...
    def draw_ego_path(self):
        ...
        if self.ego_path is not None:
            pts = np.array([[self.pixel(p) for p in self.ego_path[1:]]]) # Exclude first point, which is in ego current state.
    ...
    def draw_exo_network_state(self):
        ...
        for agent in self.network_agents:
            ...
            if actor is not None:
                ...
                if bounding_box is not None and all(p is not None for p in bounding_box) and self.bounds_min is not None and self.bounds_max is not None:

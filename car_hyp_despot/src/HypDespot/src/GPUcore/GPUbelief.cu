...
/*
Dvc_Belief::Dvc_Belief(const Dvc_DSPOMDP* model) :
	model_(model) {
}

Dvc_Belief::~Dvc_Belief() {
}

string Dvc_Belief::text() const {
	return "AbstractBelief";
}

ostream& operator<<(ostream& os, const Dvc_Belief& belief) {
	os << (&belief)->text();
	return os;
}

vector<Dvc_State*> Dvc_Belief::Sample(int num, vector<Dvc_State*> particles,
	...
	for (int i = 0; i < num; i++) {
		...
		Dvc_State* particle = model->Copy(particles[pos]);
	...
	logd << "[Dvc_Belief::Sample] Sampled " << sample.size() << " particles"
		<< endl;
...
vector<Dvc_State*> Dvc_Belief::Resample(int num, const vector<Dvc_State*>& belief,
	const Dvc_DSPOMDP* model, Dvc_History history, int hstart) {
	double unit = 1.0 / num;
	double mass = Random::RANDOM.NextDouble(0, unit);
	int pos = 0;
	...
	vector<Dvc_State*> sample;
	int count = 0;
	double max_wgt = Dvc_Globals::NEG_INFTY;
	int trial = 0;
	while (count < num && trial < 200 * num) {
		...
		Dvc_State* particle = model->Copy(belief[pos]);
	...
	logd << "[Dvc_Belief::Resample] Resampled " << sample.size() << " particles"
		<< endl;
	for (int i = 0; i < sample.size(); i++) {
		logv << " " << i << " = " << *sample[i] << endl;
	}
...
vector<Dvc_State*> Dvc_Belief::Resample(int num, const Dvc_DSPOMDP* model,
	const Dvc_StateIndexer* indexer, int action, OBS_TYPE obs) {
	if (indexer == NULL) {
		loge << "[Dvc_Belief::Resample] indexer cannot be null" << endl;
		exit(1);
	}
	...
	for (int s = 0; s < indexer->NumStates(); s++) {
		const Dvc_State* state = indexer->GetState(s);
		double prob = model->ObsProb(obs, *state, action);
		if (prob > 0) {
			Dvc_State* particle = model->Copy(state);
...
vector<Dvc_State*> Dvc_Belief::Resample(int num, const Dvc_Belief& belief, Dvc_History history,
	int hstart) {
	double reward;
	OBS_TYPE obs;
	...
	while (count < num || trial < 200 * num) {
		...
		Dvc_State* particle = particles[pos];
...
Dvc_ParticleBelief::Dvc_ParticleBelief(vector<Dvc_State*> particles, const Dvc_DSPOMDP* model,
	Dvc_Belief* prior, bool split) :
	Dvc_Belief(model),
	...
	if (fabs(Dvc_State::Weight(particles) - 1.0) > 1e-6) {
		loge << "[Dvc_ParticleBelief::Dvc_ParticleBelief] Particle weights sum to " << Dvc_State::Weight(particles) << " instead of 1" << endl;
		exit(1);
	}
	...
	if (split) {
		...
		if (particles_.size() < num_particles_) {
			logi << "[Dvc_ParticleBelief::Dvc_ParticleBelief] Splitting " << particles_.size()
				<< " particles into " << num_particles_ << " particles." << endl;
			vector<Dvc_State*> new_particles;
			int n = num_particles_ / particles_.size();
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < particles_.size(); j++) {
					Dvc_State* particle = particles_[j];
...
Dvc_ParticleBelief::~Dvc_ParticleBelief() {
	for (int i = 0; i < particles_.size(); i++) {
		model_->Free(particles_[i]);
	}
...
void Dvc_ParticleBelief::state_indexer(const Dvc_StateIndexer* indexer) {
	state_indexer_ = indexer;
}

const vector<Dvc_State*>& Dvc_ParticleBelief::particles() const {
	return particles_;
}

vector<Dvc_State*> Dvc_ParticleBelief::Sample(int num) const {
	return Dvc_Belief::Sample(num, particles_, model_);
}

void Dvc_ParticleBelief::Update(int action, OBS_TYPE obs) {
	history_.Add(action, obs);

	vector<Dvc_State*> updated;
	double total_weight = 0;
	double reward;
	OBS_TYPE o;
	// Update particles
	for (int i = 0; i <particles_.size(); i++) {
		Dvc_State* particle = particles_[i];
	...
	logd << "[Dvc_ParticleBelief::Update] " << updated.size()
	...
	if (num_effective_particles < num_particles_ / 2.0) {
		vector<Dvc_State*> new_belief = Dvc_Belief::Sample(num_particles_, particles_,
			model_);
		for (int i = 0; i < particles_.size(); i++)
			model_->Free(particles_[i]);
...
Dvc_Belief* Dvc_ParticleBelief::MakeCopy() const {
	vector<Dvc_State*> copy;
	for (int i = 0; i < particles_.size(); i++) {
		copy.push_back(model_->Copy(particles_[i]));
	}

	return new Dvc_ParticleBelief(copy, model_, prior_, split_);
}

string Dvc_ParticleBelief::text() const {
	ostringstream oss;

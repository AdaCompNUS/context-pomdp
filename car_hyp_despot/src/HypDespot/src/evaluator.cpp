...
bool Evaluator::RunStep(int step, int round) {
	...
	logi << "[RunSteuseGPU_p] Time spent in ExecuteAction(): " << (end_t - start_t)
		<< endl;
	start_t = get_time_second();
	*out_ << "-----------------------------------Round " << round
				<< " Step " << step << "-----------------------------------"
...
double Evaluator::AverageActionFrequency(int action) const
{
	return actions_count[action]/(double)discounted_round_rewards_.size();
}
...
double IPPCEvaluator::EndRound(bool terminal) {
	double start_t = get_time_second();
...
void POMDPEvaluator::InitRound() {
	...
	if(FIX_SCENARIO==1)
	{
		ifstream fin;fin.open("StartState.txt", ios::in);
		state_ = model_->ImportState(fin);
		fin.close();

		/*ofstream fout;fout.open("StartState_copy.txt", ios::trunc);
	...
	if(FIX_SCENARIO==2)
	{
		ofstream fout;fout.open("StartState.txt", ios::trunc);
		model_->ExportState(*state_,fout);
		fout.close();
	}
...
double Evaluator::SuccessRate() const
{
	double rate=0;
	for (int i = 0; i < round_reach_terminal.size(); i++) {
		bool terminal = round_reach_terminal[i];
		rate += (int)terminal;
	}
	rate=rate/(double)round_reach_terminal.size();
	return round_reach_terminal.size() > 0 ? rate : 0.0;
}

void Evaluator::PrintStatisticResult()
{
	solver_->PrintStatisticResult();
}

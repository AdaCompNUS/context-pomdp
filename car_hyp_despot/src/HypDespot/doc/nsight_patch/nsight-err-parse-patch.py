...
def patchCProject(pathToCProject):
    ...
    for c in list(node): # iteratre children
        if c.tag == 'extension' and c.attrib['id'] == 'nvcc.errorParser':
            ...
            break
    if needsPatch:
        print('patching: ' + pathToCProject, file=sys.stderr)
        patch = ET.fromstring(errorParserXml)
        node.insert(0, patch)
        tree.write(pathToCProject)
    else:
        print('up-to-date: ' + pathToCProject, file=sys.stderr)
...
def patchProject(pathToProject):
    ...
    for n in root.findall('buildSpec/buildCommand/arguments/dictionary/*'):
        if n.tag == 'key' and n.text == 'org.eclipse.cdt.core.errorOutputParser':
            #print('key '+n.text, file=sys.stderr)
            nextIsVal = True
            continue
        elif nextIsVal and n.tag == 'value':
            #print('val '+n.text, file=sys.stderr)
            node = n
            break
    ...
    if node.text.startswith("nvcc.errorParser"):
        print('up-to-date: ' + pathToProject, file=sys.stderr)
    else:
        print('patching: ' + pathToProject, file=sys.stderr)
        ...
        #print('new parsers ' + newParsers, file=sys.stderr)
        node.text = newParsers
        tree.write(pathToProject)
    #print(node.text, file=sys.stderr)

def usage():
    print('usage:', file=sys.stderr)
    print('nisight-err-parse-patch.py path-to-project', file=sys.stderr)
    print('', file=sys.stderr)
    print('Patches a eclipse project file with nvcc error parsing settings', file=sys.stderr)

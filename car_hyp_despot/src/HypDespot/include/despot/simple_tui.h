...
const option::Descriptor usage[] = {
  ...
  { E_EXPLORE_CONST, 0, "", "econst", option::Arg::Required,
		"  \t--econst <arg>  \tExploration constant (default 0.95)." },

 /* { E_ROLLOUT, 0, "", "rollout", option::Arg::Required,
		"  \t--rollout <arg>  \tRollout type (INDEPENDENT/GRAPH) (default GRAPH)." },*/
  { 0, 0, 0, 0, 0, 0 }
};
...
public:
 ...
 // virtual void InitializedGPUModel(std::string rollout_type)=0;
  virtual void InitializedGPUModel(std::string rollout_type, DSPOMDP* Hst_model=NULL)=0;
  virtual void InitializeGPUGlobals()=0;
  virtual void InitializeGPUPolicyGraph(PolicyGraph* hostGraph)=0;
  virtual void DeleteGPUModel()=0;
  virtual void DeleteGPUGlobals()=0;
  virtual void DeleteGPUPolicyGraph()=0;
  ...
  void OptionParse(option::Option* options, int& num_runs,
                   ...
                   std::string& solver_type, bool& search_solver, bool& useGPU);
...
//protected:
	void ReleaseGPUData(DSPOMDP* model, Solver* solver);
	void PrepareGPUData(DSPOMDP* model, Solver* solver);
